...
@epochs epochs Flux.train!(loss, params(model), train, ADAM(learn_rate), cb=throttle(evalcb, 10))

train       = DataLoader(train_x, train_y, batchsize=batchsize2, shuffle=true)
test        = DataLoader(test_x, test_y, batchsize=batchsize2)
println("[Training Start 2!!!]")
println("\tTraining size: $(size(train_x)[end])")
println("\tTest     size: $(size(test_x)[end])")
println("\tBatch    size: $(batchsize2)")
println("\tThreads count: $(Threads.nthreads())")
@epochs epochs2 Flux.train!(loss, params(model), train, ADAM(learn_rate), cb=throttle(evalcb, 10))
