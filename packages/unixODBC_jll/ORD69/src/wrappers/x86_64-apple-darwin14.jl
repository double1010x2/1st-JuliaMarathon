# Autogenerated wrapper script for unixODBC_jll for x86_64-apple-darwin14
...
const libodbccr_splitpath = ["lib", "libodbccr.2.dylib"]
...
const libodbccr = "@rpath/libodbccr.2.dylib"


# Relative path to `libodbcinst`
const libodbcinst_splitpath = ["lib", "libodbcinst.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libodbcinst = "@rpath/libodbcinst.2.dylib"


# Relative path to `libodbc`
const libodbc_splitpath = ["lib", "libodbc.2.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libodbc = "@rpath/libodbc.2.dylib"
